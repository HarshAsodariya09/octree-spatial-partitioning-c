...
int main(){
    ...
    *oldPoint = initialcenter;
    while (1) {
        printf("Enter command (i: insert, d: delete, s: search, r: range query, n: nearest neighbor, f: free, c: collision, q: quit): ");
        scanf(" %c", &query);
        bool isChanged = true;
        switch (query) {
            case 'i': {
            ...
            case 'r': {
                ...
                int count = 0;
                rangeQuery(root, &min, &max, &count, stdout);
            ...
            case 'f': freeTree(root); break;
            case 'c': {
                Point moving_point;
                printf("Enter moving point coordinates (x y z): ");
                scanf("%f %f %f", &moving_point.x, &moving_point.y, &moving_point.z);
                if (detect_collision(root, moving_point, COLLISION_SIZE)) {
